...
<ul class="ps-tab-list">
    ...
    <li><a href="#tab-3">Trabajador</a></li>
    <li><a href="#tab-4">Comentarios (<?php
    if ($commentJobs != null && $commentJobs > 0 ) {
        echo $commentJobs;
    } else {
        echo "0";
    }
...
<div class="ps-tabs preloadFalse">
    <div class="ps-tab active" id="tab-1">
        <div class="ps-document">
            <?php echo $producter->description_job; ?>
        </div>
    </div>
    <div class="ps-tab" id="tab-2">
        <div class="table-responsive">
            <table class="table table-bordered ps-table ps-table--specification">
                <tbody>
                    <?php $details = json_decode($producter->details_job, true); ?>
    ...
    <div class="ps-tab" id="tab-3">
        <div class="media mb-5">
            <img class="mr-5 mt-1 rounded-circle" style="width: 120px;" src="img/users/<?php echo $workerter->id_user; ?>-<?php echo $workerter->username_user; ?>/<?php echo $workerter->picture_user; ?>" alt="<?php echo $workerter->displayname_user; ?>">
            <div class="media-body ml-3 ">
                <h5 class="mt-0"><?php echo $workerter->displayname_user; ?></h5>
                <p> <?php echo $workerter->about_worker; ?></p>
            </div>
        </div>
        <a class="mt-3" href="<?php echo $path . $workerter->url_worker; ?>">Otros trabajos...</a>
    </div>
    <!-- RESEÑAS GLOBALES -->
        ...
        <div class="row">
            <div class="col-lg-5 col-12 ">
                <div class="ps-block--average-rating">
                    ...
                    <h4>Escribe un comentario sobre este trabajo</h4>
                        <form method="POST" class="needs-validation" novalidate>
                            <input type="hidden" name="idProduct" value="<?php echo $producter->id_job; ?>">
                            <?php if(isset($_SESSION["user"])): ?>
                                <input type="hidden" name="idUser" value="<?php echo $_SESSION["user"]->id_user; ?>" >
                            <?php else: ?>
                                <input type="hidden" name="idUser" value="">
                            <?php endif; ?>
                            <input type="hidden" name="idStore" value="<?php echo $workerter->id_worker; ?>">
                            <input type="hidden" name="nameStore" value="<?php echo $workerter->displayname_user; ?>">
                            <input type="hidden" name="emailStore" value="<?php echo $workerter->email_worker; ?>">
                            <div class="input-group">
                                <input 
                                class="form-control" 
                                type="text" 
                                name="Request" 
                                placeholder="Mi opinion de este trabajo es..."
                                required
                                pattern = '[-\\(\\)\\=\\%\\&\\$\\;\\_\\*\\"\\#\\?\\¿\\!\\¡\\:\\.\\,\\0-9a-zA-ZñÑáéíóúÁÉÍÓÚ ]{1,}'
                                onchange="validatejs(event, 'parrafo')">
                                <div class="valid-feedback"></div>
                                <div class="invalid-feedback">Debes escribir tu comentario</div>
            ...
            <div class="col-xl-7 col-lg-7 col-md-12 col-sm-12 col-12 ">
                ...
                <h4>Comentarios mas relevantes</h4>
                <?php if ($commentJobs != null && $commentJobs > 0) : ?>
                    <?php 
                    if($commentJobs>5){
                        $starAt = rand(0, ($commentJobs - 5));
                    }else{
                        $starAt=0;
                    }
                    ?>
                     <?php
                        $select = "id_comment,picture_user,displayname_user,id_user,username_user,content_comment";
                        $url = CurlController::api()."relations?rel=comments,jobs,users&type=comment,job,user&linkTo=id_job_comment&equalTo=". $producter->id_job ."&orderBy=Id_comment&orderMode=ASC&startAt=".$starAt."&endAt=5&select=".$select;
                        $method = "GET";
                        $fields = array();
                        $headers = array();
                        $infoUser = CurlController::request($url,$method,$fields,$headers);
                    ?>
                    <?php if($infoUser->status== 200): ?>
                        <?php if( is_array($infoUser->result) && $infoUser->total >0): ?>
                            <?php foreach($infoUser->result as $key => $value): ?>
                                <div class="media border p-3 mb-3">
                                    <?php if ($value->picture_user == null || $value->picture_user == "") : ?>
                                        <img class="mr-5 mt-1 rounded-circle" style="width: 80px;" src="img/users/default/default.png" alt="<?php echo $value->displayname_user; ?>">
                                    <?php else: ?>
                                        <img class="mr-5 mt-1 rounded-circle" style="width: 80px;" src="img/users/<?php echo $value->id_user; ?>-<?php echo $value->username_user; ?>/<?php echo $value->picture_user; ?>" alt="<?php echo $value->displayname_user; ?>">
                                        <?php //if($infoUser->method_user == "direct"): ?>
                                            <?php //if($infoUser->picture_user == ""): ?>
                                                <!-- <img class="mr-5 mt-1 rounded-circle" style="width: 80px;" src="img/users/default/default.png" alt="<?php //echo $infoUser->result[0]->name_user; ?>"> -->
                                            <?php //else: ?>
                                    ...
                                    <div class="media-body ml-3 ">
                                        <?php //if (empty($allReview[$value]["user"])) : ?>
                                            <h4 class="mt-0"><?php echo $value->displayname_user; ?></h4>
                                        <?php //else: ?>
