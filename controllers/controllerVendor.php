...
class ControllerVendor{
    public function newVendor(){
        if(isset($_POST["nameStore"])){
            if(preg_match('/^[A-Za-z0-9ñÑáéíóúÁÉÍÓÚ ]{1,}$/', $_POST["nameStore"]) &&
                ...
                preg_match('/^[-\\(\\)\\=\\%\\&\\$\\;\\_\\*\\"\\#\\?\\¿\\!\\¡\\:\\.\\,\\0-9a-zA-ZñÑáéíóúÁÉÍÓÚ ]{1,}$/', $_POST["categoryProduct"]) &&
                preg_match('/^[-\\(\\)\\=\\%\\&\\$\\;\\_\\*\\"\\#\\?\\¿\\!\\¡\\:\\.\\,\\0-9a-zA-ZñÑáéíóúÁÉÍÓÚ ]{1,}$/', $_POST["subcategoryProduct"]) &&
                preg_match('/^[-\\(\\)\\=\\%\\&\\$\\;\\_\\*\\"\\#\\?\\¿\\!\\¡\\:\\.\\,\\0-9a-zA-ZñÑáéíóúÁÉÍÓÚ ]{1,}$/', $_POST["municipioStore"])
            ){
                ...
                if(
                    ...
                    !empty($_FILES['portStore']["tmp_name"]) 
                        ...
                        if($_POST["countryStore"] != "Mexico"){
                            echo '
                                <script>
                                    formatearAlertas();
                                    notiAlert(3, "Error: No hay alcance en esta zona");
                                </script>'; 
                            return;
                        }
                        $URLworker = $_SESSION["user"]->id_user.explode("_", $_POST["categoryProduct"])[0].explode("_", $_POST["subcategoryProduct"])[0]."_".$_SESSION["user"]->username_user;
                        $imageLogo = $_FILES['logoStore'];
                        $folderLogo = "img/users";
                        $pathLogo = $URLworker;
                        $widthLogo = 270;
                        $heigthLogo = 270;
                        $nameLogo = "logo";
                        ...
                        if($saveImageLogo != 'error'){
                            ...
                            if($saveImagePort != 'error'){
                                ...
                                $dataStore = array(
                                    "id_user_worker" => $_SESSION["user"]->id_user,
                                    "id_category_worker" => explode("_", $_POST["categoryProduct"])[0],
                                    "id_subcategory_worker" => explode("_", $_POST["subcategoryProduct"])[0],
                                    "name_worker" => TemplateController::capitalize( $_POST["nameStore"]),
                                    "url_worker" => $URLworker,
                                    "logo_worker" => $saveImageLogo,
                                    "cover_worker" => $saveImagePort,
                                    "about_worker" => $_POST["infoStore"],
                                    "abstract_worker" => substr($_POST["infoStore"],0,100)."...",
                                    "email_worker" => $_POST["emailStore"],
                                    "country_worker" => explode("_", $_POST["countryStore"])[0],
                                    "city_worker" => $_POST["cityStore"],
                                    "phone_worker" => explode("_", $_POST["countryStore"])[1]."_".$_POST["phoneOrder"],
                                    "municipio_worker" => $_POST["municipioStore"],
                                    "address_worker" => $_POST["addresStore"],
                                    "map_worker" =>  json_encode($mapaStore),
                                    "socialnetwork_worker" => $sosialNetwork,
                                    "products_worker" => 1,
                                    "title_list_worker" => explode("_", $_POST["subcategoryProduct"])[1],
    ...
    static public function newProduct($idStore, $idCategory, $idSubcategory, $URLworker){
        if(isset($_POST["nameProduct"])){
            if(preg_match('/^[A-Za-z0-9ñÑáéíóúÁÉÍÓÚ ]{1,}$/', $_POST["nameProduct"])){
                ...
                $string_output=mb_strtolower($_POST["nameProduct"], 'UTF-8');
                ...
                $URLjob = $idStore.$_SESSION["user"]->id_user.$idCategory.$idSubcategory."_".$string_output;
                ...
                if(isset($_POST["galeryProduct"])&& !empty($_POST["galeryProduct"])){ 
                    ...
                    if(count($galeryArrayProduct) == $count){
                        ...
                        // ){
                            ...
                            $dataProduct = array(
                                "approval_job" => "review",
                                "feedback_job" => "tu publicacion esta en revision",
                                "state_job" => "show",
                                "id_worker_job" => $idStore,
                                "id_user_job" => $_SESSION["user"]->id_user,
                                "name_job" => TemplateController::capitalize($_POST["nameProduct"]),
                                "url_job" => $URLjob,
                                "id_category_job" => $idCategory,
                                "id_subcategory_job" => $idSubcategory,
                                "description_job" => $_POST["descriptionProduct"],
                                "summary_job" => json_encode($inputSummary),           
                                "tags_job" => json_encode( explode(",", $_POST['tagsinput'])),
                                "image_job" => $saveImagePortProduct,
                                "gallery_job" => json_encode($galeryArrayProduct),
                                "video_job" => $video_product,
                                "date_create_job" => $dateCreate
                            );
                            ...
                            $url = CurlController::api()."jobs?token=".$_SESSION["user"]->token_user;
                            $method = "POST";
                            $fields = $dataProduct;
                            $header = array(
                             "Content-Type" => "application/x-www-form-urlencoded"
                             ...
                             if($saveProduct->status == "200"){
                                ...
                                <script>
                                    formatearAlertas();
                                    switAlert("success", "El registro se realizo correctamente", "' . TemplateController::path().'acount&my-work",1500);
                                    window.location="' . TemplateController::path() . 'acount&my-work";
                                </script>'; 
                            return;
                             }else{
                                 echo '
